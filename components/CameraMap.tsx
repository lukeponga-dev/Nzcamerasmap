...
import { TrafficCamera, Severity } from '../types';
...
const CameraMap: React.FC<CameraMapProps> = ({ 
  ...
  const toggleLayer = async (layerId: string) => {
    ...
    if (newLayers.has(layerId)) {
      ...
      if (layerId === 'weather' && weatherLayerRef.current) mapRef.current.removeLayer(weatherLayerRef.current);
      if (layerId === 'transport' && transportLayerRef.current) mapRef.current.removeLayer(transportLayerRef.current);
      if (layerId === 'hazards' && hazardLayerRef.current) mapRef.current.removeLayer(hazardLayerRef.current);
    ...
    try {
      if (layerId === 'weather') {
        ...
        const icon = L.divIcon({
          html: `<div class="w-12 h-12 rounded-full bg-blue-500/10 border border-blue-500/30 backdrop-blur-md flex items-center justify-center animate-pulse"><span class="text-[10px] font-black text-blue-400">${data.temp}</span></div>`,
        ...
        const markers = hubs.map((hub: any) => {
          const icon = L.divIcon({
            html: `<div class="w-8 h-8 rounded-lg bg-emerald-900/20 border border-emerald-500/30 flex items-center justify-center"><svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 17a2 2 0 100 4 2 2 0 000-4zm11 0a2 2 0 100 4 2 2 0 000-4zM7 17h10V9H7v8zM7 9l1-2h8l1 2M5 19h14"/></svg></div>`,
            className: 'transport-icon',
            iconSize: [32, 32]
          });
          return L.marker([hub.lat, hub.lng], { icon }).bindPopup(`<div class="text-[10px] font-black text-white p-2 uppercase">${hub.name}</div>`);
        });
        ...
        const icon = L.divIcon({
          html: `<div class="w-10 h-10 rounded-full bg-red-600 border-2 border-white flex items-center justify-center shadow-2xl animate-bounce"><svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg></div>`,
  ...
  useEffect(() => {
    ...
    clusterGroupRef.current = L.markerClusterGroup({
      ...
      iconCreateFunction: (cluster: any) => {
        ...
        return L.divIcon({
          html: `<div class="relative w-10 h-10 flex items-center justify-center"><div class="absolute inset-0 rounded-full bg-blue-600/20 animate-pulse border border-blue-500/30"></div><span class="text-[11px] font-black text-blue-400 z-10">${count}</span></div>`,
    ...
    cameras.forEach(camera => {
      ...
      const iconHtml = `
        <div class="relative w-8 h-8 rounded-full border-2 border-white/10 flex items-center justify-center bg-[#09090b] shadow-xl overflow-visible" style="border-color: ${isOrigin || isDest ? color : 'rgba(255,255,255,0.1)'}">
    ...
    <div className="relative w-full h-full bg-[#09090b] overflow-hidden">
            ...
            {[
              ...
              <button
                ...
                className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all active:scale-90 relative ${
                  activeLayers.has(layer.id) ? 'bg-blue-600 text-white shadow-lg' : 'bg-zinc-900/50 text-zinc-500 hover:text-zinc-300'
                }`}
              >
                {isLayerLoading === layer.id ? (
                  <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>
                ) : (
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={layer.icon}/></svg>
                )}
              </button>
            ))}
         </div>
      ...
      <div className="absolute top-24 left-6 z-[400] flex flex-col gap-4 w-full max-w-sm">
        <form onSubmit={handleAddressSearch} className="relative group">
          <input 
            type="text" 
            placeholder="Tactical Location Search..." 
            value={addressQuery}
        ...
        <button onClick={locateUser} className="w-fit bg-[#0c0a09]/95 backdrop-blur-xl border border-white/10 rounded-2xl px-5 py-3 flex items-center gap-3 shadow-2xl hover:border-blue-500/50 transition-all active:scale-95">
          <svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
          <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Uplink Position</span>
        </button>
      </div>
        ...
        <div className="absolute top-24 right-24 z-[400] animate-in slide-in-from-right-10 pointer-events-none">
           <div className="bg-[#0c0a09]/95 backdrop-blur-3xl border border-blue-500/30 p-5 rounded-3xl shadow-2xl flex flex-col gap-5 pointer-events-auto min-w-[240px]">
              <div className="flex justify-between items-center">
                 <h4 className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Route Intelligence</h4>
                 <div className="flex gap-2">
                    ...
                    <button onClick={handleClearRoute} className="p-2 bg-zinc-900 rounded-lg border border-zinc-800 text-red-500 transition-colors hover:bg-red-900/20"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12"/></svg></button>
                 </div>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-end border-b border-zinc-800/50 pb-2">
                  <div className="flex flex-col"><span className="text-[8px] font-black text-zinc-600 uppercase">Dist</span><span className="text-xl font-black text-blue-400 mono">{routeStats.distance}</span></div>
                  <div className="flex flex-col items-end"><span className="text-[8px] font-black text-zinc-600 uppercase">Base</span><span className="text-xl font-black text-emerald-400 mono">{routeStats.duration}</span></div>
                </div>
                {routeStats.aiPredicted && (
                  <div className="bg-blue-600/10 border border-blue-500/20 rounded-2xl p-3">
                    <div className="flex items-center justify-between mb-1"><span className="text-[9px] font-black text-blue-500 uppercase tracking-widest">Tactical Forecast</span></div>
                    <div className="flex justify-between items-baseline"><span className="text-2xl font-black text-white mono">{routeStats.aiPredicted}</span><span className="text-[8px] font-bold text-blue-400/60 uppercase">{routeStats.aiFactor}</span></div>
                  </div>
                )}
              </div>
              <button onClick={() => setShowBriefing(!showBriefing)} className="w-full py-2 bg-zinc-900 text-[9px] font-black text-blue-500 uppercase rounded-xl border border-zinc-800 transition-all">
                {showBriefing ? 'Hide Intel' : 'Show Intel'}
              </button>
           </div>
        </div>
      )}
      ...
      {routeBriefing && showBriefing && (
        <div className="absolute inset-x-0 bottom-0 sm:bottom-auto sm:top-24 sm:left-1/2 sm:transform sm:-translate-x-1/2 z-[500] w-full max-w-xl px-0 sm:px-4 animate-in slide-in-from-bottom sm:zoom-in-95 duration-500">
           <div className="bg-[#09090b]/98 border-t sm:border border-blue-500/40 p-6 sm:p-8 rounded-t-[2rem] sm:rounded-[3rem] shadow-2xl backdrop-blur-4xl relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-4"><h3 className="text-[10px] font-black text-blue-500 uppercase tracking-[0.5em]">Sector Briefing</h3></div>
              ...
              <button onClick={() => setShowBriefing(false)} className="mt-4 w-full text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] hover:text-zinc-400">Dismiss</button>
           </div>
        </div>
      )}
    </div>
